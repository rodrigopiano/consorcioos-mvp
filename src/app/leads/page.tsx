...
import {
  Users, Search, Plus, AlertTriangle, MessageCircle, Phone, ChevronRight
} from "lucide-react";
import { mockLeads } from "@/lib/mock-data";
import { FunnelStage, STAGE_LABELS, INTEREST_LABELS, ORIGIN_LABELS, CHANNEL_LABELS } from "@/lib/types";
              ...
              return (
                <tr key={lead.id} className="hover:bg-[#273549] transition-colors">
                  ...
                  <td className="px-4 py-3 hidden md:table-cell">
                    <span className="text-xs text-slate-300">{INTEREST_LABELS[lead.interest]}</span>
                  </td>
                  <td className="px-4 py-3 hidden lg:table-cell">
                    <span className="text-xs text-slate-400">{ORIGIN_LABELS[lead.origin]}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className={cn("text-xs px-2 py-1 rounded-full font-medium", stageColor[lead.stage])}>
                  ...
                  <td className="px-4 py-3 hidden lg:table-cell">
                    {lead.value ? (
                      <span className="text-xs text-green-400 font-medium">{formatCurrency(lead.value)}</span>
                    ) : (
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      ...
                      <button className="p-1.5 text-slate-400 hover:text-white hover:bg-[#334155] rounded transition-colors">
