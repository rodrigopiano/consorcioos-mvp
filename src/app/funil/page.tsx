"use client";
import { Kanban, AlertTriangle, TrendingUp } from "lucide-react";
import { mockLeads } from "@/lib/mock-data";
import { FunnelStage, STAGE_LABELS, INTEREST_LABELS } from "@/lib/types";
...
export default function FunilPage() {
  const totalValue = mockLeads.reduce((sum, l) => sum + (l.value || 0), 0);
  const salesValue = mockLeads
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            {mockLeads.length} leads · Pipeline total: {formatCurrency(totalValue)} · Vendido: {formatCurrency(salesValue)}
          </p>
        </div>
      </div>
      ...
      <div className="bg-[#1e293b] rounded-xl border border-[#334155] p-5">
        ...
        <div className="space-y-2">
          {FUNNEL_ORDER.map((stage, i) => {
            ...
            const firstCount = mockLeads.filter(l => l.stage === FUNNEL_ORDER[0]).length || 1;
            const width = Math.max((count / mockLeads.length) * 100, 5);
            return (
              <div key={stage} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-6 bg-[#334155] rounded overflow-hidden">
                  <div
                    className={cn("h-full rounded flex items-center px-2", stageDotColors[stage])}
                    style={{ width: `${width}%`, opacity: 0.8 }}
                  >
                    <span className="text-xs font-bold text-white">{count}</span>
                  </div>
                </div>
                <div className="w-10 text-xs text-slate-400">{count}x</div>
              </div>
            );
          })}
        </div>
    ...
    <div className={cn("w-52 bg-[#1e293b] rounded-xl border border-[#334155] border-t-4 flex-shrink-0", stageColors[stage])}>
      <div className="p-3 border-b border-[#334155]">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs bg-[#334155] text-slate-300 px-1.5 py-0.5 rounded-full font-medium">
            {leads.length}
          </span>
        </div>
        {value > 0 && (
          <div className="text-xs text-slate-400 mt-0.5">{formatCurrency(value)}</div>
        )}
      </div>
      <div className="p-2 space-y-2 max-h-96 overflow-y-auto">
        {leads.length === 0 && (
          <div className="text-center py-6 text-xs text-slate-600">Vazio</div>
        )}
        {leads.map(lead => {
          ...
          return (
            <div
              key={lead.id}
              className={cn(
                "bg-[#273549] rounded-lg p-2.5 cursor-pointer hover:bg-[#2d3e54] transition-colors border",
                urgency === "overdue" ? "border-orange-500/40" :
                urgency === "lost" ? "border-red-800/40" : "border-transparent"
              )}
            >
              ...
              {lead.nextAction && (
                <div className="text-xs text-slate-400 mt-1.5 leading-tight line-clamp-2">
              ...
              {lead.nextAction && (
                <div className={cn(
                  ...
                  urgency === "overdue" ? "text-orange-400" : "text-slate-500"
                )}>
